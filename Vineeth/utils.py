    ...
    for local_round in local_rounds:
        ...
        for method in methods:
            ...
            for experiment in log_dict.keys():
                ...
                plt.plot(np.arange(ROUNDS), mean_accuracy_profile, color='#CC4F1B', label=f'{method} - {experiment}')
                plt.fill_between(np.arange(ROUNDS), mean_accuracy_profile - std_dev_accuracy_profile, mean_accuracy_profile + std_dev_accuracy_profile, alpha=0.5)
        ...
        plt.tight_layout()
        plt.savefig(f'plots/local_rounds_accuracy_{dataset}_{local_round}.jpg')
        plt.show()
...
def plot_accuracry_stacked_errobar_plot(path, dataset):
    ...
    for local_round in local_rounds:
        ...
        plt.errorbar(np.arange(len(key_list)), np.array(final_acc_mean_list), np.array(final_acc_std_list), fmt='ok', lw=3)
        plt.errorbar(np.arange(len(key_list)), np.array(final_acc_mean_list), [np.array(final_acc_mean_list) - np.array(final_acc_min_list), np.array(final_acc_max_list) - np.array(final_acc_mean_list)],
        ...
        plt.tight_layout()
        plt.savefig(f'plots/local_rounds_accuracy_stacked_{dataset}_{local_round}.jpg')
        plt.show()
